...
On top of the simulation engine, the harness defines a planned Streamlit visualization layer. The Docker runtime includes the required Python dashboard dependencies and a \texttt{docker compose} dashboard profile that expects \texttt{ui/dashboard/app.py}; however, the \texttt{ui/} implementation is not present in the current repository snapshot. The intended visualization layer exposes:
...
\begin{itemize}
  \item Python 3.11 with \texttt{litellm>=1.50}, \texttt{duckdb}, \texttt{networkx}, and \texttt{streamlit}. \texttt{camel-ai>=0.2.90} is retained as an optional Plan-B layer for framework-managed agent state or tool calls.
