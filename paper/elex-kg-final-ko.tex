...
시뮬레이션 엔진 위에 하네스는 FastAPI 백엔드(\texttt{backend/})와 Vite + React 프런트엔드(\texttt{frontend/})로 구성된 시각화 계층을 정의한다. 두 서비스 모두 \texttt{docker-compose.yml}에 등록되어 있다. 의도된 시각화 계층은 다음을 노출한다.
...
\begin{itemize}
  \item Python 3.11, \texttt{litellm>=1.50}, \texttt{duckdb}, \texttt{networkx}, \texttt{fastapi}; 시각화 프런트엔드는 별도의 Vite + React 앱이다. 의존성은 \texttt{uv}(\texttt{pyproject.toml} + \texttt{uv.lock})로 관리한다. \texttt{camel-ai>=0.2.90}은 framework 관리 에이전트 상태나 도구 호출이 필요한 경우를 위한 선택적 Plan B 계층으로 유지된다.
