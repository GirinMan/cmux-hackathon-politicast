...
PolitiKAST 활용 포인트: 엔진 아키텍처 비교표의 근거로 사용한다. PolitiKAST의 `src/data`, `src/llm`, `src/sim`, `src/kg`, `backend/`(FastAPI) + `frontend/`(React) 구조를 MiroFish의 frontend/backend/simulation/memory layer와 나란히 보여줄 수 있다.
...
PolitiKAST 활용 포인트: 한국어 발표 준비, 논문 supplementary note, React frontend의 "비교 대상: MiroFish" 설명에 활용한다.
...
PolitiKAST 활용 포인트: React frontend sidebar의 run status, API key/capacity warning, frozen artifact viewer, troubleshooting note 설계에 참고한다.
...
PolitiKAST 활용 포인트: `_workspace/snapshots/`와 React frontend의 artifact browser 설계에 참고한다.
